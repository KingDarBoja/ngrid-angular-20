...
function hasListeners(source: any): boolean {
  // Angular 14 to 19 compatibility - check if EventEmitter has subscribers
  if (!source) return false;

  // Try Angular 16+ way first
  if (typeof source.observed === 'boolean') {
    return source.observed;
  }

  // Try method-based detection
  if (typeof source.hasListeners === 'function') {
    return source.hasListeners();
  }

  // Try Angular 14 way (observers array)
  if (source.observers && Array.isArray(source.observers)) {
    return source.observers.length > 0;
  }

  // Fallback: assume listeners exist to avoid blocking events
  // This ensures detail rows work even if detection fails
  return true;
}

function findEventSource(
  source: Event,
): { type: 'row' | 'cell'; target: HTMLElement } | undefined {
  ...
  if (cellTarget) {
    ...
    return { type: 'row', target: source.target as any };
  }
}
...
export class PblNgridTargetEventsPlugin<T = any> {
  ...
  mouseDown = new EventEmitter<
    Events.PblNgridCellEvent<T, MouseEvent> | Events.PblNgridRowEvent<T>
  >();
  mouseUp = new EventEmitter<
    Events.PblNgridCellEvent<T, MouseEvent> | Events.PblNgridRowEvent<T>
  >();
  keyUp = new EventEmitter<
    Events.PblNgridCellEvent<T, KeyboardEvent> | Events.PblNgridRowEvent<T>
  >();
  ...
  private setupDomEvents(): void {
    ...
    const createCellEvent = <TEvent extends Event>(
      cellTarget: HTMLElement,
      source: TEvent,
    ): Events.PblNgridCellEvent<T, TEvent> | undefined => {
      const rowTarget = cellTarget.parentElement;
      const matrixPoint = matrixRowFromRow(
        rowTarget,
        cdkTable._rowOutlet.viewContainer,
      );
      if (matrixPoint) {
        const event: Events.PblNgridCellEvent<T, TEvent> = {
          ...matrixPoint,
          source,
          cellTarget,
          rowTarget,
        ...
        if (matrixPoint.type === 'data') {
          (event as Events.PblNgridDataMatrixPoint<T>).row =
            grid.ds.renderedData[matrixPoint.rowIndex];
          ...
          const db =
            event.type === 'header'
              ? metaRowService.header
              : metaRowService.footer;
        ...
        */
        ...
        if (matrixPoint.subType === 'data') {
          ...
          event.column = column;
          (event as Events.PblNgridDataMatrixPoint<T>).context =
            this.pluginCtrl.extApi.contextApi.getCell(
              event.rowIndex,
              columnIndex,
            );
          ...
          const rowInfo = (
            matrixPoint.type === 'header'
              ? store.metaHeaderRows
              : store.metaFooterRows
          )[event.rowIndex];
          ...
          if (rowInfo.isGroup) {
            ...
            event.column =
              matrixPoint.type === 'header' ? record.header : record.footer;
          }
        }
        return event;
      }
    ...
    const createRowEvent = <TEvent extends Event>(
      rowTarget: HTMLElement,
      source: TEvent,
      root?: Events.PblNgridCellEvent<T, TEvent>,
    ): Events.PblNgridRowEvent<T> | undefined => {
      if (root) {
        ...
        if (root.type === 'data') {
          ...
          (event as Events.PblNgridDataMatrixRow<T>).context =
            root.context.rowContext;
        }
        return event;
      } else {
        const matrixPoint = matrixRowFromRow(
        ...
        if (matrixPoint) {
          ...
          if (matrixPoint.type === 'data') {
            const row = this.pluginCtrl.extApi.contextApi.getRow(
              matrixPoint.rowIndex,
            );
            if (!row) {
              return undefined;
            }
    ...
    const emitCellLeave = (
      source: MouseEvent,
    ): Events.PblNgridCellEvent<T> | undefined => {
      if (lastCellEnterEvent) {
        const lastCellEnterEventTemp = lastCellEnterEvent;
        this.cellLeave.emit(
          Object.assign({}, lastCellEnterEventTemp, { source }),
        );
    ...
    const emitRowLeave = (
      ...
      if (lastRowEnterEvent) {
        ...
        this.rowLeave.emit(
          Object.assign({}, lastRowEnterEventTemp, { source }),
        );
    ...
    const splitProcessedEvent = <TEvent extends Event>(
      event: ReturnType<typeof processEvent>,
    ) => {
      const cellEvent =
        event.type === 'cell'
          ? (event.event as Events.PblNgridCellEvent<T, TEvent>)
          : undefined;
      const rowEvent = cellEvent
        ? createRowEvent<TEvent>(
            cellEvent.rowTarget,
            cellEvent.source,
            cellEvent,
          )
    ...
    const registerUpDownEvents = <TEvent extends Event>(
      eventName: string,
      emitter: EventEmitter<
        Events.PblNgridCellEvent<T, TEvent> | Events.PblNgridRowEvent<T>
      >,
    ) => {
      fromEvent(cdkTableElement, eventName)
        .pipe(
          takeUntil(this.destroyed),
          // Temporarily bypass hasListeners check
    ...
    */
    const clickStream = fromEvent(cdkTableElement, 'click').pipe(
      takeUntil(this.destroyed),
      // Temporarily bypass hasListeners check to force event processing
    ...
    fromEvent(cdkTableElement, 'mouseleave')
      ...
      .subscribe((source: MouseEvent) => {
        let lastEvent:
          | Events.PblNgridRowEvent<T>
          | Events.PblNgridCellEvent<T> = emitCellLeave(source);
    ...
    fromEvent(cdkTableElement, 'mousemove')
      ...
      .subscribe((source: MouseEvent) => {
        ...
        const lastCellTarget =
          lastCellEnterEvent && lastCellEnterEvent.cellTarget;
        ...
        const rowTarget =
          (cellEvent && cellEvent.rowTarget) ||
          (isRowContainer(source.target as any) && (source.target as any));
  ...
  private syncRow<TEvent extends Event>(
    event: Events.PblNgridRowEvent<T> | Events.PblNgridCellEvent<T, TEvent>,
  ): void {
...
@Directive({
  ...
  selector:
    'pbl-ngrid[targetEvents], pbl-ngrid[mouseDown], pbl-ngrid[mouseUp], pbl-ngrid[rowClick], pbl-ngrid[rowDblClick], pbl-ngrid[rowEnter], pbl-ngrid[rowLeave], pbl-ngrid[cellClick], pbl-ngrid[cellDblClick], pbl-ngrid[cellEnter], pbl-ngrid[cellLeave], pbl-ngrid[keyDown], pbl-ngrid[keyUp]',
...
export class PblNgridTargetEventsPluginDirective<T>
  extends PblNgridTargetEventsPlugin<T>
