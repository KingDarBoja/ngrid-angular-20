...
- A single entry point into `@perbula/ngrid`, `@perbula/ngrid-material` and `@perbula/ngrid-bootstrap`
...
- For convinience, legacy theming API is still availavle at `@perbula/ngrid/theming`, `@perbula/ngrid-material/theming` and `@perbula/ngrid-bootstrap/theming`. The legacy API will be removed in v5.0.0
...
* **ngrid:** move most of utils to `@perbula/ngrid/core` ([2990511](https://github.com/shlomiassaf/ngrid/commit/2990511c5bf3e68716bf2c08c87577fd6b2dc946))
...
* **ngrid:** To reduce clutter in the main packge the configuration symbols moved to `@perbula/ngrid/core`.
...
* **ngrid:** To reduce clutter in the main packge the datasource symbols (all) and some of the column model symbols moved to `@perbula/ngrid/core`.
...
* **ngrid:** If you've used `unrx` in your code, it is not located in `@perbula/ngrid/core` since it is not for used by "regular" users, only required for plugin authoring.
* **ngrid:** Pagination types have moved from `@perbula/ngrid` to `@perbula/ngrid/core` as part of the cleanup process to simplify the main package. `@perbula/ngrid` re-exports the interfaces and types but the concrete implementations for `PblPagingPaginator` and `PblTokenPaginator` are now in the core pacakge as they are
...
Filenames change should not have any effect unless you are extending the types using TS augmentation so any `declare module '@perbula/ngrid/lib/table/...` will have to change.
...
All SCSS theme files are now located in the core package (@perbula/ngrid)
